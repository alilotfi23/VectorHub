...
- **Multi-tenancy:** every resource (collections, API keys, audit logs) is scoped by `tenant_id`. Enforce isolation at the query layer (never trust client-supplied tenant_id; derive it from the authenticated principal). Support tenant-level resource quotas (max collections, max vectors, max QPS).
- **Audit logging:** every write operation (collection create/delete, vector upsert/delete, role changes, API key creation) writes an immutable audit record (actor, tenant, action, resource, timestamp, result) to a dedicated `audit_log` table; expose `GET /audit-logs` for admins with filtering. Enforce immutability at the DB level, not just app convention: the app's Postgres role gets `INSERT`/`SELECT` only on `audit_log` (no `UPDATE`/`DELETE` grants), so a bug or compromised app credential can't alter history. Set this up as part of the Alembic migration/DB provisioning in Phase 1 or 2.
...
- These three limits are part of the standard vector record/query contract and must be documented in the OpenAPI schema (Pydantic `Field` constraints + descriptions), not just enforced silently.
...
- `VECTOR_*` — e.g. `VECTOR_NOT_FOUND`, `VECTOR_DIMENSION_MISMATCH`, `VECTOR_DIMENSION_EXCEEDED`, `BATCH_SIZE_EXCEEDED`, `TOP_K_EXCEEDED`
- `JOB_*` — e.g. `JOB_NOT_FOUND`, `JOB_FAILED`
...
- `VALIDATION_*` — generic Pydantic validation failures not covered by a more specific code above
...
- **Docker Compose (local/dev):** full stack including self-hosted Qdrant, Weaviate, Milvus (standalone, via its docker-compose with etcd+minio), and Chroma. Use multi-stage Dockerfile, non-root user, healthchecks.
...
- **Cloud-managed mode:** config-driven — each adapter reads its connection info from env vars/secret store, so the same app image can run against Qdrant Cloud, Weaviate Cloud, Zilliz Cloud (managed Milvus), with self-hosted Chroma as the only one needing a server. Document this clearly in `.env.example`.
...
1. [ ] **Scaffold** — project structure, `.gitignore`, git init, config management (pydantic-settings), `pre-commit` hooks (`ruff` + `mypy`), Postgres models, Alembic, initial commits.
2. [ ] **Auth & RBAC** — register/login/JWT/refresh, roles, API keys, tenant model, permission-checking dependency.
3. [ ] **Adapter interface + Chroma adapter first** (simplest, easiest to test locally) — prove the abstraction end-to-end with collections + vectors + query routes.
4. [ ] **Qdrant and Weaviate adapters** — including hybrid search and filtering, expand integration tests via testcontainers.
5. [ ] **Milvus adapter** — partitions, index types, consistency levels, expand integration tests via testcontainers.
6. [ ] **Capability matrix endpoint, async batch jobs** (`arq`), audit logging middleware, Redis rate limiting.
7. [ ] **Observability** — structured logs, Prometheus metrics, tracing, Sentry.
8. [ ] **Deployment artifacts** — Dockerfile, docker-compose (full + cloud variants), k8s manifests/Helm, CI pipeline.
9. [ ] **Docs & polish** — OpenAPI tags/examples, README with architecture diagram and setup instructions, load test (k6/Locust) script for at least one adapter.

## Progress Log

_Update this section at the end of each phase. Newest entry on top._

- **2026-08-14** — Pre-build design session (brainstorming) complete, no code written yet. Resolved five architecture decisions that amend this file: (1) tenant isolation = per-backend native tenancy matrix (Weaviate/Qdrant native tenants, Milvus partitions, Chroma per-tenant collections); (2) hybrid search = client-supplied sparse vectors + normalized `alpha`; (3) async batches staged via object storage (MinIO/S3, JSONL), never through arq/Redis; (4) control-plane drift = observable non-goal (`backend_status` field, disjoint exists/missing/error); (5) audit immutability = app/migrator Postgres roles + all-role guard trigger, stated threat model. Full design: `docs/superpowers/specs/2026-08-14-vectorhub-platform-architecture-design.md` — read it before Phase 1 and fold the decisions into this file when Phase 1 kicks off.
