...
3. [ ] **Adapter interface + Chroma adapter first** (simplest, easiest to test locally) — prove the abstraction end-to-end with collections + vectors + query routes; Chroma exercises the per-tenant-collection tenancy model, opaque physical naming, and `backend_status` on the read path.
