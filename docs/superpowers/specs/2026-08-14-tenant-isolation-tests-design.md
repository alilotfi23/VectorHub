...
- **Qdrant:** **drift from the original design — Qdrant removed the native tenant API (`multi_tenancy_config`/`create_tenant`) from client and server (verified against v1.19: the server 404s the old endpoints).** The implemented mechanism is payload-partition: a `_vhk_tenant_id` payload field with a `keyword` index marked `is_tenant=True`. Same-ID seed; scoped query returns only that tenant's points; an unfiltered query over a collection carrying the `is_tenant` index proves the tenant filter is load-bearing (both tenants' indistinguishable points would come back — the adapter's always-applied filter is what closes it). Assert the `is_tenant` index exists on `_vhk_tenant_id` (mechanism present), the physical collection name is the opaque `col_<uuid>`, and `ensure_tenant` is idempotent.
...
- **Milvus:** partition-per-tenant. Inserts route by `partition_name`; search with `partition_names=[A]` → only A's. **Drift, recorded when implemented (Phase 5, verified against server 3.0.0): a raw search WITHOUT `partition_names` does NOT return nothing — it spans all partitions and returns every tenant's indistinguishable points.** The strongest honest proofs are therefore: search `partition_names=["_default"]` → empty (inserts routed to named partitions — the default partition holds nothing), a raw unscoped search returns both tenants' rows (the leak the adapter's always-applied `partition_names` scope closes), a nonexistent partition name errors (code 1100, fail-closed), and insert into an uncreated partition → error. `ensure_tenant` idempotency (create_partition twice).
