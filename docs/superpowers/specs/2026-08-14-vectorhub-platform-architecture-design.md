...
| **Qdrant** | Native multi-tenancy | Collection created with `multi_tenancy_config`; `create_tenant` per tenant; queries pass `tenant` — dedicated shard per tenant | `ensure_tenant`: idempotent `create_tenant` before first use | Qdrant ≥ 1.10 |
...
| **4 — Qdrant & Weaviate** | Native multi-tenancy (version gates: Qdrant ≥ 1.10, Weaviate ≥ 1.20); `ensure_tenant` adapter op; tenant-scoped query/upsert; hybrid contract (§5) incl. sparse schema + `alpha` mapping |
